...
import vn.edu.husc.taphoa2hand_backend.dto.request.PostsDTO.PaymentMethodRequest;
...
public interface PostsMapper {
    ...
    PostDetailResponse toPostDetailResponse(Posts post); // Hàm này sẽ map toàn bộ thông tin từ Posts sang PostDetailResponse, bao gồm cả các trường con như PostDetail, PostAddress, PostImage, PaymentMethodEnum, và Categories


    @Mapping(target = "categories", ignore = true) // MapStruct sẽ không tự động map Set<Categories> sang Set<CategoryResponse>, chúng ta sẽ xử lý thủ công trong Service
    @Mapping(target = "acceptedPaymentMethods", ignore = true) // MapStruct sẽ không
    @Mapping(target = "postImages", ignore = true) // MapStruct sẽ không tự động map List<PostImage> sang List<PostImageResponse>, chúng ta sẽ xử lý thủ công trong Service
    @Mapping(target = "postAddress", ignore = true) // MapStruct sẽ không tự
    @Mapping(target = "user", ignore = true) // MapStruct sẽ không tự động map Users sang UsersResponse, chúng ta sẽ xử lý thủ công trong Service
    @Mapping(target = "postDetail", source = "postDetail") // MapStruct sẽ tự động map PostDetail sang PostDetailInfoResponse nếu chúng ta có hàm chuyển đổi tương ứng
    PostDetailResponse toDetailResponse(Posts post); // Hàm này sẽ map toàn bộ thông tin từ Posts sang PostDetailResponse, bao gồm cả các trường con như PostDetail, PostAddress, PostImage, PaymentMethodEnum, và Categories

    PostDetail toPostDetail(PostDetailResponse postDetailResponse); // Hàm này sẽ map toàn bộ thông tin từ PostDetailResponse sang PostDetail, bao gồm cả các trường con như PostDetailInfoResponse, PostAddressResponse, PostImageResponse, PaymentMethodResponse, và CategoryResponse
    PostAddress toPostAddress(PostAddressResponse postAddressResponse); // Hàm này sẽ map toàn bộ thông tin từ PostAddressResponse sang PostAddress
    List<PostImage> toPostImage(List<PostImageResponse> postImageResponses); // Hà
    @Mapping(target = "acceptedPaymentMethods", ignore = true) // MapStruct sẽ không tự động map List<PaymentMethodResponse> sang List<PaymentMethodEnum>, chúng ta sẽ xử lý thủ công trong Service
    @Mapping(target = "id", ignore = true)
    Posts toPosts(PostDetailResponse postDetailResponse); // Hàm này sẽ map toàn bộ thông tin từ PostDetailResponse sang Posts, bao gồm cả các trường con như PostDetail, PostAddress, PostImage, PaymentMethodEnum, và Categories
@Mapping(target = "id", ignore = true)
